...
class BayesianOptimization(LightningModule):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__()
        self.gmodel = getcls(model.model._target_)
    ...
    def update_gp_model(self):
        self.model = self.gmodel(self.train_x, self.train_y)
        mll = self.mll(self.model.likelihood, self.model)
        self.fit(mll)
    ...
    def on_train_start(self) -> None:
        ...
        train_y_value = self.train_metric(self.img_x, self.img_target)
        self.train_y = torch.tensor([train_y_value], dtype=torch.float32).unsqueeze(-1) # Define the bounds of your optimization based on PCA variances (assuming normality and using some scale factor)
    ...
    def training_step(self, batch: Any, batch_idx: int) -> Any:
        ...
        with torch.enable_grad():
            acqf = self.acqf(self.model,beta=2)
            ...
            new_y = self.train_metric(self.img_x, self.img_target)
            ...
            self.train_y = torch.cat([self.train_y, new_y_tensor.unsqueeze(0)]).detach()
