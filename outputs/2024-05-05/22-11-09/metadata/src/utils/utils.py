...
def getcls(full_class_string):
    """
    Dynamically imports a class from a full class string.
    
    Args:
    full_class_string (str): The full path to the class, formatted as module.submodule.ClassName
    
    Returns:
    class: The class object specified by full_class_string.
    """
    # Split the full_class_string by dots
    class_parts = full_class_string.split('.')
    
    # Extract the module path and class name
    module_path = '.'.join(class_parts[:-1])  # Join all parts except the last as the module path
    class_name = class_parts[-1]  # The last part is the class name
...
# Custom resolver that adds a directory to sys.path if it exists
def add_to_sys_path_if_exists(path):
...
def concat_path(base_path, extension):
    if base_path:
...
def register_custom_resolvers(
    ...
    if not OmegaConf.has_resolver("add_to_sys_path"):
        # Register the custom resolver
        OmegaConf.register_new_resolver("add_to_sys_path", add_to_sys_path_if_exists)

    if not OmegaConf.has_resolver("concat_path"):
        # Register the custom resolver
        OmegaConf.register_new_resolver("concat_path", concat_path)
