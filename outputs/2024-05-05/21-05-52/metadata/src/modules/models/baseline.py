...
class BayesianOptimization(LightningModule):
    ...
    def update_gp_model(self):
        ...
        self.mll = self.mell(self.model.likelihood, self.model)
        self.fit(self.mll)
    ...
    def on_train_start(self) -> None:
        ...
        pca_sample = self.sampler.sample_pca_space(1)
        self.train_x = torch.tensor(pca_sample)
        self.train_y = torch.tensor([self.train_metric(self.train_x,self.target)]).unsqueeze(-1)
    ...
    def training_step(self, batch: Any, batch_idx: int) -> Any:
        ...
        with torch.enable_grad():
            ...
            new_y = self.train_metric(candidates.detach().numpy(),self.target)
